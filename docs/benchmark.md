...
This benchmark tests a single MCP server with 7 tools (936 tokens of schema). In isolation, that overhead is modest. In practice, however, agent configurations rarely use a single server. A typical setup might aggregate a filesystem server, a git server, a database server, and one or more domain-specific servers — each contributing its own tool definitions. Schema overhead scales linearly with the number of servers: five servers averaging 7 tools each could inject 4,000–6,000 tokens of schema into the context. Because the agent runtime replays tool schemas on every turn, this cost compounds multiplicatively with turn count. A 15-turn session with 5 MCP servers could spend 60,000–90,000 tokens on schema replay alone — context capacity that would otherwise hold conversation history, file contents, or model reasoning. Additional tools also increase model decision complexity, which can add turns and further amplify the effect. CLI tools accessed via an existing Bash tool add zero incremental schema regardless of how many are installed.
